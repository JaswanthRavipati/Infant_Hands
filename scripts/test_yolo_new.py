...
from config import YOLO_WEIGHTS as MODEL_PATH, TEST_VIDEO_NEW_DIR, TEST_OUTPUT_DIR, CONF_THRESHOLD, INFER_IMG_SIZE
...
videos = [v for v in os.listdir(TEST_VIDEO_NEW_DIR) if v.lower().endswith((".mp4", ".avi", ".mov", ".mkv"))]

log.info("Videos found: %s", videos)

for video in videos:

    video_path = os.path.join(TEST_VIDEO_NEW_DIR, video)
